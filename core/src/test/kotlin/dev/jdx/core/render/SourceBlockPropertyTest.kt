...
/**
 * Generative invariants for the source renderer (T-023, TESTING.md §4 — the
 * standing bar demands a generating family, not only examples).
 */
class SourceBlockPropertyTest {
    ...
    fun `rendering is byte-identical across runs`() = runBlocking<Unit> {
        checkAll(
            ...
            val start = 1
            val first = buildSourceBlock(
                "com.example.A", "com.example.A", "com/example/A.java",
                file, start, end, provenanceOf(), contextLines = context,
                lineNumbers = numbers, maxLines = max,
            )
            val second = buildSourceBlock(
                "com.example.A", "com.example.A", "com/example/A.java",
                file, start, end, provenanceOf(), contextLines = context,
                lineNumbers = numbers, maxLines = max,
            )
            first.renderText() shouldBe second.renderText()
            first.toJson(command = "source") shouldBe second.toJson(command = "source")
        }
    }
