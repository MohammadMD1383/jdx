...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 34) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names DONE) |
| **Next task** | **T-066** (make `ReadCommandsTest` hermetic to the machine's workspaces) — lowest open TODO; T-068 (identical-root dedupe), T-069 (`--repo` flag) also open. T-065 (JFR-style `$$` names) closed in session 34 with a dedicated `UNNAMEABLE_CLASS` warning — model invariant untouched, differential/soak harnesses count the family as degradation; `test`+`tier2Test` green (672 + 268 tests, 0 failures). |
...
| `docs/progress/sessions-031-040.md` | 31–33 | open |
