...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 32) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe DONE); M2 index hardening (T-064 speed, T-065 `$$` names) is next |
| **Next task** | **T-064** (close the indexer 3,000/s end-to-end gap) — lowest open TODO; T-065 (`$$` names), T-066 (`ReadCommandsTest` hermeticity), T-068 (identical-root dedupe), T-069 (`--repo` flag) also open. T-063 (share the fixture-jar resolution helpers) closed in session 32 — `FixtureJars` in testfixtures' `testFixtures` source set (canonical resolution + L-029 guard + `classNames`/`classBytes`, 5-test tier-2 suite); all four golden-suite copies migrated (index `RendererGoldenTest`, cli `ReadCommandsGoldenTest` + `SortOrdersGoldenTest`); core's `Fixtures` stays with the mirror rule documented (L-058); tier-1 18.9 s of 30 s. |
...
| `docs/progress/sessions-031-040.md` | 31– | open |
