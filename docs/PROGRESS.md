...
| | |
|---|---|
| **Last updated** | 2026-09-17 (session 25) |
...
| **Next task** | **T-019** (Maven coordinates) or **T-057** (fault injection) / **T-058** (metamorphic) — lowest unblocked TODOs after T-018; T-062 (`--sort` orders), T-063 (share fixture-jar helpers), T-064 (indexer speed), T-065 (`$$` names), T-066 (`ReadCommandsTest` hermeticity), T-068 (identical-root dedupe) also open. T-018 (`cache`) and T-067 (config cache) closed in session 25. |
...
| **Docs** | Append-only logs sharded (D-023): sessions → `docs/progress/` (session 25 in `sessions-021-030.md`), decisions → `docs/decisions/` (shard 2 open — D-030 discovery semantics/deviations), lessons → `docs/lessons/` (`L-026-050.md` nearly full — L-046 (settings catalog), L-047 (config-cache report), L-048 (root-flag chain), L-049 (stale provisioned JDK); one slot left). Every hard-won lesson goes to `docs/LESSONS.md` (D-024). |
| **Blocked on** | Nothing. Sessions 24–25 unpushed (`15c0cc8`…`1a7d601`, incl. T-017, T-018, T-067); **push still needs owner go-ahead per session (D-012)**. |
...
| `docs/progress/sessions-021-030.md` | 21–25 | open (5 free) |
