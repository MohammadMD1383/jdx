...
| | |
|---|---|
| **Last updated** | 2026-09-20 (session 45) |
...
| **Next task** | **T-027** (`javap` engine — T-026 leaves `--engine javap` as exit 3 naming it; T-028 remains an M3 one-liner; T-072 `--with-doc` enrichment filed TODO). |
| **Task count** | 72 tasks defined (T-001…T-072) — M0–M2 DONE in full detail; M3 started (T-071, T-021…T-026 DONE; T-027…T-028 one-liners TODO; T-072 `--with-doc` TODO); M4–M7 coarse TODO |
...
| **Test status** | **All green, no shelving needed** (`test`+`tier2Test` green — incl. all JaCoCo line gates and the T-026 suites: decompile `DecompileCacheKeyTest`+`VineflowerStagingTest`+`VineflowerVersionTest` (tier 1) and `VineflowerDecompilerTest`+`DecompiledSliceTest` (tier 2), index `BodyServiceTest`/`SourceServiceTest` fallback/forced/fake-fault/determinism additions, cli `BodyCommandTest`/`SourceCommandTest` flag plumbing + `BodyCommandsServiceTest`/`SourceCommandsServiceTest` end-to-end (incl. the system-cache path); session 38 closed T-070, so `:cli:tier2Test` passes with the ambient `fx` workspace in place and with `JDX_WORKSPACE=fx` forced); `./gradlew mutationTest` tier 4 green — `core` 91 % mutation vs the build-failing 80 gate, `index` 70 % measured/reported, `sources`/`decompile` 0 mutants (T-071's arrival gives `sources` real code — mutation-measured from here on); `./gradlew soak` green (2m 38s solo, session 45; was green 4m 12s solo in session 44; was green 2m 20s solo in session 42; was green 2m 23s solo in session 41; was green 3m 51s solo in session 40 except the known `JavapCorpusSoakTest` reds, now closed as T-065 degradation-skips). T-060 added: build wiring (`mutationTest` entry point, per-module `pitest` blocks, line gates on `test`+`tier2Test` exec) + ~1,230 lines of killer tests across 14 `core` test files (each names its mutant) + 2 production cleanups (dead `advance()`, redundant `isTarget` param). **Caveat (sessions 34/38/39/41/42/44/45):** `verifyTier1Budget` is red on this machine (pre-existing machine variance in environment-sensitive suites — `JdkLayoutTest`, `DoctorEnvironmentTest`, plus `JavaBodiesTest` slowness in session 42's 64.6 s run — not a test failure: 0 failures/errors in every run; session 45's new suites stay out of the slowest-10). |
| **Docs** | Append-only logs sharded (D-023): sessions → `docs/progress/` (`sessions-041-050.md` open at 43), decisions → `docs/decisions/` (shard 2 open — D-036 source semantics), lessons → `docs/lessons/` (`L-051-075.md` open at L-074). Every hard-won lesson goes to `docs/LESSONS.md` (D-024). |
