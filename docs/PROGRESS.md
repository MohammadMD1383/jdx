...
| **Phase** | Design complete; **M0–M6 DONE**; **M7 TODO** (T-047…T-052 coarse, T-060 done early; T-080/T-081 low priority) |
| **Next task** | **M7 T-047…T-052** (coarse one-liners — expand into detail blocks when starting, per the board rule). |
| **Task count** | 82 tasks (T-001…T-082): all DONE except the coarse M7 one-liners (T-060 done early), plus T-080/T-081 (filed session 65, low priority) |
...
| **Docs** | Sharded append-only logs (D-023): sessions → `docs/progress/` (`sessions-051-060.md` full at 60, `sessions-061-070.md` full at 70, `sessions-071-080.md` open at 73), decisions → `docs/decisions/` (shard `D-051-075.md` open at D-059), lessons → `docs/lessons/` (`L-076-100.md` full at L-100, `L-101-125.md` open at L-104). New decisions/lessons go to the newest shard, never into session entries. |
