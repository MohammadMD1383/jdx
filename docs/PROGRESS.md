...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 37) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names DONE, T-066 test hermeticity DONE, T-068 identical-root dedupe DONE, T-069 `--repo` DONE) |
| **Next task** | **T-070** (tier-2 read-command hermeticity) — the only open TODO. T-069 closed in session 37: repeatable `--repo` on all seven read commands + `ws create` (explicit → stored → Central-last fetch order, exit-3 URL validation, loopback-proven; D-034 supersedes D-032 §6). |
