...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 36) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names DONE, T-066 test hermeticity DONE, T-068 identical-root dedupe DONE) |
| **Next task** | **T-069** (`--repo` flag, depends T-019 DONE) — lowest open TODO; T-070 (tier-2 read-command hermeticity) also open. T-068 closed in session 36: `JdxService.openRoots` now dedupes roots by normalised absolute path (first occurrence wins, shading kept; `search`/`resolve`/`ls`/`tree` list each symbol once for one file passed twice). |
