...
| | |
|---|---|
| **Last updated** | 2026-09-21 (session 57) |
| **Repository** | <https://github.com/MohammadMD1383/jdx> (public, Apache-2.0) |
| **Phase** | Design complete; **M0–M4 DONE**, **M5 open** (T-035 DONE, T-076 DONE, T-036 WIP via T-077 next), M6–M7 TODO |
| **Next task** | **M5 T-077** (suspend + `@JvmName` signature repair over the T-076 `KmClass` carrier; then T-078 properties/default-args) |
| **Task count** | 76 tasks (T-001…T-076): all DONE except T-036 (WIP), T-074 (`srcmap` remainder), T-075 (`usages` graph enrichment), and the coarse M5–M7 one-liners (T-060 done early, T-035 done) |
...
| **Docs** | Sharded append-only logs (D-023): sessions → `docs/progress/` (`sessions-051-060.md` open at 57), decisions → `docs/decisions/` (shard 2 at D-049), lessons → `docs/lessons/` (`L-076-100.md` open at L-088). New decisions/lessons go to the newest shard, never into session entries. |
