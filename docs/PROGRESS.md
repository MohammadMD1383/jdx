...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 35) |
| **Repository** | <https://github.com/MohammadMD1383/jdx> (public, Apache-2.0) |
| **Phase** | Design complete; **M0 done (test spine complete), M1 done (read path), M2 hardening next** |
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names DONE, T-066 test hermeticity DONE) |
| **Next task** | **T-068** (dedupe identical resolved roots before querying) — lowest open TODO; T-069 (`--repo` flag) and T-070 (tier-2 read-command hermeticity, split out of T-066) also open. T-066 closed in session 35 (test-only: every `ReadCommandsTest` command construction now injects `InMemoryWorkspaceStore()` + empty env — `:cli:test` 29/29 green with ambient `fx` present, incl. `JDX_WORKSPACE=fx` forced). |
| **Task count** | 68 tasks defined (T-001…T-070; M0–M2 in full detail, M3–M7 as one-liners) |
...
| **Test status** | **All green** (`test`+`tier2Test` green — 672 + 268 tests, 0 failures — incl. all four JaCoCo line gates; `./gradlew mutationTest` tier 4 green — `core` 91 % mutation vs the build-failing 80 gate, `index` 70 % measured/reported, `sources`/`decompile` 0 mutants; `./gradlew soak` green except the known `JavapCorpusSoakTest` reds, now closed as T-065 degradation-skips) — **only with `~/.config/jdx` shelved**, see caveats. T-060 added: build wiring (`mutationTest` entry point, per-module `pitest` blocks, line gates on `test`+`tier2Test` exec) + ~1,230 lines of killer tests across 14 `core` test files (each names its mutant) + 2 production cleanups (dead `advance()`, redundant `isTarget` param). **Caveat 1 (L-038, T-066/T-070):** `:cli:test` (tier 1) now passes with the ambient `fx` workspace present (T-066, session 35); a bare `check` still needs `~/.config/jdx` shelved for `:cli:tier2Test` — `ReadCommandsServiceTest` 15/18, both read golden suites and the `ReadCommandDiscoveryTest` end-to-end fail with `exit 5 / no artifacts match: testfixtures/...` from the same ambient-glob trap (proven stashed-clean, filed as T-070). **Caveat 2 (session 34):** `verifyTier1Budget` is red stashed-clean too (31.4 s vs the 30 s gate; 45.0 s with the T-065 tree) — pre-existing machine variance in environment-sensitive suites (`DoctorEnvironmentTest`, `JdkLayoutTest`), not a test failure: 0 failures/errors in every run. |
| **Docs** | Append-only logs sharded (D-023): sessions → `docs/progress/` (`sessions-031-040.md` open at 34), decisions → `docs/decisions/` (shard 2 open — D-033 sort orders), lessons → `docs/lessons/` (`L-051-075.md` open at L-060). Every hard-won lesson goes to `docs/LESSONS.md` (D-024). |
...
| `docs/progress/sessions-031-040.md` | 31–35 | open |
