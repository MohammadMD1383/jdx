...
| | |
|---|---|
| **Last updated** | 2026-09-21 (session 47) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening **complete** (T-065 `$$` names DONE, T-066 test hermeticity DONE, T-068 identical-root dedupe DONE, T-069 `--repo` DONE, T-070 tier-2 hermeticity DONE); **M3 bodies nearly done (T-071 `sources` `SourceRoot` DONE — first slice of T-020; T-021 JavaParser body extraction DONE; T-022 `jdx body` DONE; T-023 `jdx source` DONE; T-024 `jdx signature` DONE; T-025 `jdx doc` DONE; T-026 Vineflower decompiler fallback DONE; T-027 `javap` engine DONE; T-028 `SOURCES_VERSION_MISMATCH` DONE)** |
| **Next task** | **T-072** (`--with-doc` enrichment — lowest TODO by number with deps DONE; T-073 Vineflower→javap auto-fallback likewise TODO; then M4 T-029…T-034 coarse). |
| **Task count** | 73 tasks defined (T-001…T-073) — M0–M2 DONE in full detail; M3 nearly done (T-071, T-021…T-028 DONE; T-072 `--with-doc` TODO; T-073 auto-fallback TODO); M4–M7 coarse TODO |
