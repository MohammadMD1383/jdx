...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names DONE, T-066 test hermeticity DONE, T-068 identical-root dedupe DONE, T-069 `--repo` DONE, **T-070 tier-2 hermeticity DONE**) |
| **Next task** | **No open TODO remains** (T-001…T-070 all DONE — session 38 closed T-070: isolated `InMemoryWorkspaceStore()` + null-env in all four tier-2 read-command tests, T-066 precedent; full `:cli:tier2Test` green with ambient `fx` in place and `JDX_WORKSPACE=fx` forced). Next: owner picks the milestone to expand (M3 bodies). |
| **Task count** | 70 tasks defined (T-001…T-070; M0–M2 in full detail, M3–M7 as one-liners) — **all DONE** |
...
| **Test status** | **All green, no shelving needed** (`test`+`tier2Test` green — 951 tests, 0 failures — incl. all four JaCoCo line gates; session 38 closed T-070, so `:cli:tier2Test` passes with the ambient `fx` workspace in place and with `JDX_WORKSPACE=fx` forced); `./gradlew mutationTest` tier 4 green — `core` 91 % mutation vs the build-failing 80 gate, `index` 70 % measured/reported, `sources`/`decompile` 0 mutants; `./gradlew soak` green except the known `JavapCorpusSoakTest` reds, now closed as T-065 degradation-skips). T-060 added: build wiring (`mutationTest` entry point, per-module `pitest` blocks, line gates on `test`+`tier2Test` exec) + ~1,230 lines of killer tests across 14 `core` test files (each names its mutant) + 2 production cleanups (dead `advance()`, redundant `isTarget` param). **Caveat (sessions 34/38):** `verifyTier1Budget` is red on this machine (40.3 s vs the 30 s gate; 31.4 s stashed-clean in session 34) — pre-existing machine variance in environment-sensitive suites (`DoctorEnvironmentTest`, `JdkLayoutTest`), not a test failure: 0 failures/errors in every run. |
