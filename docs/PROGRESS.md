...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 31) |
...
| **Active milestone** | M1 — Read path (T-062 sort orders DONE); M2 index hardening (T-064 speed, T-065 `$$` names) queued behind T-063 |
| **Next task** | **T-063** (share the fixture-jar resolution helpers) — lowest unblocked TODO; T-064 (indexer speed), T-065 (`$$` names), T-066 (`ReadCommandsTest` hermeticity), T-068 (identical-root dedupe), T-069 (`--repo` flag) also open. T-062 (`--sort name\|declaring` orders) closed in session 31 — `MemberSort` in `core` (kind byte-identical default, declaring alphabetical, name strictly flat via run-length groups), plumbing through `MemberFilters.sort`, 18 sort goldens (Generics, TrafficLight$1, KotlinMembers), tier-1 23.6 s of 30 s. |
...
| **Docs** | Append-only logs sharded (D-023): sessions → `docs/progress/` (`sessions-021-030.md` full at 10 — next session opens `sessions-031-040.md`), decisions → `docs/decisions/` (shard 2 open — D-032 maven semantics), lessons → `docs/lessons/` (`L-051-075.md` open at L-056). Every hard-won lesson goes to `docs/LESSONS.md` (D-024). |
