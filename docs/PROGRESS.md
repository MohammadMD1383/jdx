...
| **Phase** | Design complete; **M0–M6 DONE**; **M7 open** (T-049 DONE session 74, T-060 done early; T-047/T-048/T-050/T-051/T-052 TODO, T-080/T-081 low priority) |
| **Next task** | **M7 T-047** (token budgets — write the detail block when starting, per the board rule). |
| **Task count** | 82 tasks (T-001…T-082): all DONE except the M7 remainder (T-047/T-048/T-050/T-051/T-052 TODO; T-049 DONE session 74, T-060 done early), plus T-080/T-081 (filed session 65, low priority) |
...
| **Test status** | **Tiers 1–2 green** (`check`: 1988 tests, 0 failures, session 73); **tier 3 green except** `JavapCorpusSoakTest`, which reds only on JDK-internal synthetic `access$` members (proven pre-existing on the stashed-clean tree, same drift family as sessions 53/55/56/58; Kotlin re-inclusion via `--view jvm` contributed zero mismatches over 112 classes); mutation tier 4 green (`core` 91 % vs the 80 gate, last measured session 55; the new `dev.jdx.core.rpc` package measures 94 %, session 66). New suite this session: cli tier-2 `AdapterParityTest` (5: 17-command table + JDK sample + failure branches + CLI spot checks + 100-case hostile property) — green. **Caveat:** `verifyTier1Budget` is red on this machine (pre-existing machine variance, 0 test failures — not a test failure). |
| **Docs** | Sharded append-only logs (D-023): sessions → `docs/progress/` (`sessions-051-060.md` full at 60, `sessions-061-070.md` full at 70, `sessions-071-080.md` open at 74), decisions → `docs/decisions/` (shard `D-051-075.md` open at D-059), lessons → `docs/lessons/` (`L-076-100.md` full at L-100, `L-101-125.md` open at L-104). New decisions/lessons go to the newest shard, never into session entries. |
...
| `docs/progress/sessions-071-080.md` | 71–80 | open |
