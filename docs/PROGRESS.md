...
| | |
|---|---|
| **Last updated** | 2026-09-19 (session 33) |
...
| **Active milestone** | M1 — Read path **complete** (T-062 sort orders, T-063 helper dedupe, T-064 indexer 3,000/s gap DONE); M2 index hardening continues (T-065 `$$` names) |
| **Next task** | **T-065** (handle JFR-style `$$` class names) — lowest open TODO; T-066 (`ReadCommandsTest` hermeticity), T-068 (identical-root dedupe), T-069 (`--repo` flag) also open. T-064 (indexer 3,000/s gap) closed in session 33 with no code change — re-measured 4,004–5,900/s on `minecraft-client.jar` and `jrt:/` under JDK 26.0.2.1 (session-19's 2,502/s predates the current JDK and `BulkWriter`); `check` green, tier-1 18.9 s of 30 s. |
