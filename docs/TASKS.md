...
### T-085 — `cache gc` sweeps orphan daemon `.log` files · `DONE` (session 84)
...
*Closed in session 84. `CacheService` gains injectable `daemonRuntimeDir`
(`null` = skip, today's behaviour) + `daemonSocketAlive`; `gc()` sweeps
top-level `*.log` orphans (sibling socket absent or no daemon answers,
live-daemon logs kept, throwing probe keeps, never throws) even when the
index DB is absent, but not on exit-4/6 failure (D-066). `GcReport` gains
`daemonLogs: DaemonLogSweep` (sorted names + bytes); text renders a
`daemon logs:` line only when something was/would be deleted (seam-off output
byte-identical); JSON gains `daemonLogsDeleted` + `daemonLogBytesFreed`;
production `cacheGroup` wires the real runtime dir + `health` probe. Tests:
index tier-1 `CacheServiceTest` +8 (stale/dead deleted, live kept,
dry-run, seam-off, throwing probe, non-log/subdir ignored, missing dir,
artifacts+logs composed, lowercase-suffix pin) + new
`DaemonLogSweepPropertyTest` (200-case sorted/conservative/idempotent laws —
the generating family); cli tier-2 `CacheCommandsTest` +3 (text sweep,
JSON fields, dry-run). `check` green; live proof: dry-run/real/second-run
